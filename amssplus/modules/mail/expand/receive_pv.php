...
$url_link="file=receive_pv&index=$index&name_search=$name_search&remote_user_office=$remote_user_office&remote_user_id=$remote_user_id&remote_login_status=$remote_login_status&remote_sync=$remote_sync";  // 2_กำหนดลิงค์
...
?>
<a href="maildetail.php?id=<?php echo $id;?>&remote_user_office=<?php echo $remote_user_office;?>&remote_user_id=<?php echo $remote_user_id;?>" onclick="check(<?php echo $id;?>)" class="dialogify" title="คลิกเพื่อดูรายละเอียด"><span style="text-decoration: none"><?php echo $subject; ?></span></a>
<?php
if($sch_person_index==1){
...
function goto_display(val){
	if(val==1){
		callfrm("?file=receive_pv&index=8&switch_index=1"); 
		}
}
function check(val){
