...
$result1=mysqli_query($connect,"SELECT *,person_sch_main.name,system_school_group.name as group_name FROM  person_sch_main left join system_school on person_sch_main.school_code=system_school.school_code left join system_school_group on system_school.school_group=system_school_group.code WHERE (person_sch_main.status='0' or person_sch_main.status='9') and person_sch_main.position_code='1' and system_school.school_type='1' order by system_school.school_group") ;
...
while ($r1=mysqli_fetch_array($result1)) {
	...
	$name = $r1['name'] ;
	$surname = $r1['surname'] ;
	$group_name = $r1['group_name'] ;
	$school_name = $r1['school_name'] ;
	$fullname=$prename.$name." ".$surname."  [".$school_name." : ".$group_name."]";
...
<BR><input name="boxchecked" type="hidden" id="boxchecked" value="<?php echo $list1?>"> <input name="sd_index" type="hidden"  value="<?php echo $sd_index?>"><input name="index" type="hidden"  value="1"><input name="ref_id" type="hidden"  value="<?php echo $ref_id?>">
