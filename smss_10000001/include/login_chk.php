...
if($username=='admin'){
$sql = "select * from system_user where username='admin' and status='1' and smss_admin='1' ";
}
else{
...
//รักษาการในตำแหน่ง
$today = date("Y-m-d"); 
$sql = "select * from delegate_main where person_id='$_SESSION[login_user_id]' and ('$today'>=start) and ('$today'<=finish)";
$dbquery_delegate = mysqli_query($connect,$sql);
if($result_delegate = mysqli_fetch_array($dbquery_delegate)){
$start=$result_delegate['start'];
$finish=$result_delegate['finish'];
			...
			$day_num=getNumDay($start,$finish);
			if($day_num>=30){
			$_SESSION['login_status']=2;
			}
}	
...
if(isset($_SESSION['RMSS'])){
unset($_SESSION['RMSS']);
}
if(isset($_SESSION['PMSS'])){
unset($_SESSION['PMSS']);
}
if(isset($_SESSION['DMSS'])){
unset($_SESSION['DMSS']);
}
